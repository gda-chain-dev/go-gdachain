...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light gdachain protocol")
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light gdachain peer connected", "name", p.Name())
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light gdachain peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light gdachain message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case GetBlockHeadersMsg:
		...
		pm.server.fcCosgdaats.update(msg.Code, query.Amount, rcost)
	...
	case GetHeaderProofsMsg:
		...
		trieDb := trie.NewDatabase(gdadb.NewTable(pm.chainDb, light.ChtTablePrefix))
		for _, req := range req.Reqs {
...
func (d *downloaderPeerNotify) registerPeer(p *peer) {
	...
	pm.downloader.RegisterLightPeer(p.id, gdaVersion, pc)
}
