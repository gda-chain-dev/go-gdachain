...
import (
	...
	"github.com/gdachain/go-gdachain/les/flowcontrol"
...
func NewLesServer(gda *gda.gdachain, config *gda.Config) (*LesServer, error) {
	...
	pm, err := NewProtocolManager(gda.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, gda.EventMux(), gda.Engine(), newPeerSet(), gda.BlockChain(), gda.TxPool(), gda.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(gda.BlockChain().Genesis().Hash(), pv)
	}
...
type requestCosgdaats struct {
	lock  sync.RWMutex
	db    gdadb.Database
	stats map[uint64]*linReg
}

type requestCosgdaatsRlp []struct {
	MsgCode uint64
	Data    []byte
}

var rcStatsKey = []byte("_requestCosgdaats")

func newCosgdaats(db gdadb.Database) *requestCosgdaats {
	...
	if db != nil {
		...
		var statsRlp requestCosgdaatsRlp
		if err == nil {
	...
	return &requestCosgdaats{
		db:    db,
		stats: stats,
	}
}

func (s *requestCosgdaats) store() {
	s.lock.Lock()
	defer s.lock.Unlock()

	statsRlp := make(requestCosgdaatsRlp, len(reqList))
...
func (s *requestCosgdaats) getCurrentList() RequestCostList {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *requestCosgdaats) update(msgCode, reqCnt, cost uint64) {
	s.lock.Lock()
	defer s.lock.Unlock()
