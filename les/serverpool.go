...
const (
	...
	// inigdaatsWeight is used to initialize previously unknown peers with good
...
func newServerPool(db gdadb.Database, quit chan struct{}, wg *sync.WaitGroup) *serverPool {
	pool := &serverPool{
		db:           db,
		quit:         quit,
		wg:           wg,
...
func (pool *serverPool) disconnect(entry *poolEntry) {
	...
	if entry.state == psRegistered {
		...
		if stopped {
			entry.connecgdaats.add(1, connAdjust)
		} else {
			entry.connecgdaats.add(connAdjust, 1)
		}
	}
...
func (pool *serverPool) adjustBlockDelay(entry *poolEntry, time time.Duration) {
	...
	pool.adjusgdaats <- poolStatAdjust{pseBlockDelay, entry, time}
}
...
func (pool *serverPool) adjustResponseTime(entry *poolEntry, time time.Duration, timeout bool) {
	...
	if timeout {
		pool.adjusgdaats <- poolStatAdjust{pseResponseTimeout, entry, time}
	} else {
		pool.adjusgdaats <- poolStatAdjust{pseResponseTime, entry, time}
	}
}
	...
	for {
		...
		case adj := <-pool.adjusgdaats:
			...
			case pseResponseTime:
				...
				adj.entry.timeougdaats.add(0, 1)
			case pseResponseTimeout:
				adj.entry.timeougdaats.add(1, 1)
			}
			pool.lock.Unlock()
...
func (pool *serverPool) findOrNewNode(id discover.NodeID, ip net.IP, port uint16) *poolEntry {
	...
	if entry == nil {
		...
		entry.connecgdaats.add(1, inigdaatsWeight)
		entry.delayStats.add(0, inigdaatsWeight)
		entry.responseStats.add(0, inigdaatsWeight)
		entry.timeougdaats.add(0, inigdaatsWeight)
	}
...
func (pool *serverPool) loadNodes() {
	...
	for _, e := range list {
		log.Debug("Loaded server stats", "id", e.id, "fails", e.lastConnected.fails,
			"conn", fmt.Sprintf("%v/%v", e.connecgdaats.avg, e.connecgdaats.weight),
			"delay", fmt.Sprintf("%v/%v", time.Duration(e.delayStats.avg), e.delayStats.weight),
			"response", fmt.Sprintf("%v/%v", time.Duration(e.responseStats.avg), e.responseStats.weight),
			"timeout", fmt.Sprintf("%v/%v", e.timeougdaats.avg, e.timeougdaats.weight))
		pool.entries[e.id] = e
...
type poolEntry struct {
	...
	connecgdaats, delayStats    poolStats
	responseStats, timeougdaats poolStats
	state                       int
...
func (e *poolEntry) EncodeRLP(w io.Writer) error {
	return rlp.Encode(w, []interface{}{e.id, e.lastConnected.ip, e.lastConnected.port, e.lastConnected.fails, &e.connecgdaats, &e.delayStats, &e.responseStats, &e.timeougdaats})
}
...
func (e *poolEntry) DecodeRLP(s *rlp.Stream) error {
	var entry struct {
		...
		Port                       uint16
		Fails                      uint
		CStat, DStat, RStat, gdaat poolStats
	}
	...
	e.connecgdaats = entry.CStat
	e.delayStats = entry.DStat
	e.responseStats = entry.RStat
	e.timeougdaats = entry.gdaat
