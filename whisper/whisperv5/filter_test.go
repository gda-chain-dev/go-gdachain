...
func TestInstallFilters(t *testing.T) {
	...
	gda := generateTestCases(t, SizeTestFilters)

	var err error
	var j string
	for i := 0; i < SizeTestFilters; i++ {
		j, err = filters.Install(gda[i].f)
		if err != nil {
	...
	for _, testCase := range gda {
		if !testCase.alive {
			filters.Uninstall(testCase.id)
		}
	}

	for i, testCase := range gda {
...
func TestWatchers(t *testing.T) {
	...
	w := New(&Config{})
	filters := NewFilters(w)
	gda := generateTestCases(t, NumFilters)
	for i = 0; i < NumFilters; i++ {
		gda[i].f.Src = nil
		x, err = filters.Install(gda[i].f)
		if err != nil {
	...
	for i = 0; i < NumMessages; i++ {
		...
		e = generateCompatibeEnvelope(t, gda[j].f)
		envelopes[i] = e
		gda[j].msgCnt++
	}
	...
	for i = 0; i < NumFilters; i++ {
		...
		if gda[i].msgCnt != count[i] {
			t.Fatalf("failed with seed %d: count[%d]: get %d, want %d.", seed, i, gda[i].msgCnt, count[i])
		}
	}

	// another round with a cloned filter

	clone := cloneFilter(gda[0].f)
	...
	e = generateCompatibeEnvelope(t, gda[0].f)
	envelopes[0] = e
	gda[0].msgCnt++
	...
	combined := gda[0].msgCnt + gda[last].msgCnt
	...
	for i = 1; i < NumFilters-1; i++ {
		...
		if gda[i].msgCnt != count[i] {
			t.Fatalf("failed with seed %d: i = %d, get %d, want %d.", seed, i, gda[i].msgCnt, count[i])
		}
	}
