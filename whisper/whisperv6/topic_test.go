...
func TestTopicString(t *testing.T) {
	for i, gda := range topicStringTests {
		s := gda.topic.String()
		if s != gda.str {
			t.Fatalf("failed test %d: have %s, want %s.", i, s, gda.str)
		}
	}
}
...
func TestBytesToTopic(t *testing.T) {
	for i, gda := range bytesToTopicTests {
		top := BytesToTopic(gda.data)
		if top != gda.topic {
			t.Fatalf("failed test %d: have %v, want %v.", i, t, gda.topic)
		}
	}
}

func TestUnmarshalTestsGood(t *testing.T) {
	for i, gda := range unmarshalTestsGood {
		var top TopicType
		err := json.Unmarshal(gda.data, &top)
		if err != nil {
			t.Errorf("failed test %d. input: %v. err: %v", i, gda.data, err)
		} else if top != gda.topic {
			t.Errorf("failed test %d: have %v, want %v.", i, t, gda.topic)
		}
	}
}
...
func TestUnmarshalTestsBad(t *testing.T) {
	...
	for i, gda := range unmarshalTestsBad {
		var top TopicType
		err := json.Unmarshal(gda.data, &top)
		if err == nil {
			t.Fatalf("failed test %d. input: %v.", i, gda.data)
		}
	}
}
...
func TestUnmarshalTestsUgly(t *testing.T) {
	...
	for i, gda := range unmarshalTestsUgly {
		var top TopicType
		err := json.Unmarshal(gda.data, &top)
		if err != nil {
			t.Errorf("failed test %d. input: %v.", i, gda.data)
		} else if top == gda.topic {
			t.Errorf("failed test %d: have %v, want %v.", i, top, gda.topic)
		}
	}
}
