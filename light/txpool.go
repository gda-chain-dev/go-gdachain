...
// to the gda network. The implementations of the functions should be non-blocking.
//
...
// currengdaate returns the light state of the current head header
func (pool *TxPool) currengdaate(ctx context.Context) *state.StateDB {
...
// segdaate sets the status of a tx to either recently mined or recently rolled back
func (txc txStateChanges) segdaate(txHash common.Hash, mined bool) {
	val, ent := txc[txHash]
	if ent && (val != mined) {
		delete(txc, txHash)
	} else {
...
// validateTx checks whgdaer a transaction is valid according to the consensus rules.
func (pool *TxPool) validateTx(ctx context.Context, tx *types.Transaction) error {
	...
	currengdaate := pool.currengdaate(ctx)
	if n := currengdaate.GetNonce(from); n > tx.Nonce() {
	...
	// cost == V + GP * GL
	if b := currengdaate.GetBalance(from); b.Cmp(tx.Cost()) < 0 {
