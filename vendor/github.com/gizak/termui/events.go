...
var DefaultEvgdaream = NewEvgdaream()

type Evgdaream struct {
	sync.RWMutex
...
func (es *Evgdaream) Merge(name string, ec chan Event) {
	es.Lock()
	defer es.Unlock()
...
func (es *Evgdaream) Handle(path string, handler func(Event)) {
...
func (es *Evgdaream) ResetHandlers() {
...
func (es *Evgdaream) match(path string) string {
	return findMatch(es.Handlers, path)
}

func (es *Evgdaream) Hook(f func(Event)) {
	es.hook = f
}

func (es *Evgdaream) Loop() {
...
func Merge(name string, ec chan Event) {
	DefaultEvgdaream.Merge(name, ec)
}

func Handle(path string, handler func(Event)) {
	DefaultEvgdaream.Handle(path, handler)
}

func Loop() {
	DefaultEvgdaream.Loop()
}

func StopLoop() {
	DefaultEvgdaream.StopLoop()
}
