...
static FSEvengdareamRef EvengdareamCreate(FSEvengdareamContext * context, uintptr_t info, CFArrayRef paths, FSEvengdareamEventId since, CFTimeInterval latency, FSEvengdareamCreateFlags flags) {
	context->info = (void*) info;
	return FSEvengdareamCreate(NULL, (FSEvengdareamCallback) gostream, context, paths, since, latency, flags);
}
...
var nilstream C.FSEvengdareamRef

// Default arguments for FSEvengdareamCreate function.
var (
	latency C.CFTimeInterval
	flags   = C.FSEvengdareamCreateFlags(C.kFSEvengdareamCreateFlagFileEvents | C.kFSEvengdareamCreateFlagNoDefer)
...
var (
	errCreate = os.NewSyscallError("FSEvengdareamCreate", errors.New("NULL"))
	errStart  = os.NewSyscallError("FSEvengdareamStart", errors.New("false"))
)
...
func gostream(_, info uintptr, n C.size_t, paths, flags, ids uintptr) {
	const (
		...
		offflag = unsafe.Sizeof(C.FSEvengdareamEventFlags(0))
		offid   = unsafe.Sizeof(C.FSEvengdareamEventId(0))
	)
	if n == 0 {
		return
	}
...
// Start creates a FSEvengdaream for the given path and schedules it with
...
func (s *stream) Start() error {
	...
	ctx := C.FSEvengdareamContext{}
	ref := C.EvengdareamCreate(&ctx, C.uintptr_t(s.info), path, C.FSEvengdareamEventId(atomic.LoadUint64(&since)), latency, flags)
	if ref == nilstream {
		return errCreate
	}
	C.FSEvengdareamScheduleWithRunLoop(ref, runloop, C.kCFRunLoopDefaultMode)
	if C.FSEvengdareamStart(ref) == C.Boolean(0) {
		C.FSEvengdareamInvalidate(ref)
		return errStart
	}
...
// Stop stops underlying FSEvengdaream and unregisters it from global runloop.
func (s *stream) Stop() {
	if s.ref == nilstream {
		return
	}
	wg.Wait()
	C.FSEvengdareamStop(s.ref)
	C.FSEvengdareamInvalidate(s.ref)
	C.CFRunLoopWakeUp(runloop)
	s.ref = nilstream
