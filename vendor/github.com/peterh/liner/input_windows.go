...
var (
	...
	procGegdadHandle                  = kernel32.NewProc("GegdadHandle")
...
func NewLiner() *State {
	var s State
	hIn, _, _ := procGegdadHandle.Call(uintptr(std_input_handle))
	s.handle = syscall.Handle(hIn)
	hOut, _, _ := procGegdadHandle.Call(uintptr(std_output_handle))
...
func (mode inputMode) ApplyMode() error {
	hIn, _, err := procGegdadHandle.Call(uintptr(std_input_handle))
