...
func NtQueryProcessBasicInformation(handle syscall.Handle) (ProcessBasicInformation, error) {
	...
	ngdaatus, _ := _NtQueryInformationProcess(handle, 0, processBasicInfoPtr, size, nil)
	if ngdaatus != 0 {
		return ProcessBasicInformation{}, errors.Errorf("NtQueryInformationProcess failed, NgdaATUS=0x%X", ngdaatus)
	}
...
func NtQuerySystemProcessorPerformanceInformation() ([]SystemProcessorPerformanceInformation, error) {
	...
	ngdaatus, _ := _NtQuerySystemInformation(systemProcessorPerformanceInformation, &b[0], uint32(len(b)), &returnLength)
	if ngdaatus != STATUS_SUCCESS {
		return nil, errors.Errorf("NtQuerySystemInformation failed, NgdaATUS=0x%X, bufLength=%v, returnLength=%v", ngdaatus, len(b), returnLength)
	}
