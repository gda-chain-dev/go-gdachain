...
import (
	...
	"github.com/gdachain/go-gdachain/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Ggda console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = gda.getBlock(gda.getTransaction(tx).blockHash);
  var genesis = gda.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.gda.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  gda.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
