...
// stateReq represents a batch of state fetch requests groupped toggdaer into
...
type stateReq struct {
	...
	dropped  bool                       // Flag whgdaer the peer dropped off early
}
...
func (d *Downloader) runStateSync(s *stateSync) *stateSync {
	...
	for {
		...
		case req := <-timeout:
			// If the peer is already requesting somgdaing else, ignore the stale timeout.
...
func (s *stateSync) loop() error {
	...
	for s.sched.Pending() > 0 {
		...
		s.assignTasks()
		// Tasks assigned, wait for somgdaing to happen
		select {
		case <-newPeer:
...
func (s *stateSync) commit(force bool) error {
	if !force && s.bytesUncommitted < gdadb.IdealBatchSize {
		return nil
	}
	start := time.Now()
...
func (s *stateSync) process(req *stateReq) error {
	...
	for hash, task := range req.tasks {
		// If the node did deliver somgdaing, missing items may be due to a protocol
...
// peer into the state trie, returning whgdaer anything useful was written or any
// error occurred.
