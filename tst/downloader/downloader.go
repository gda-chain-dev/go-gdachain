...
var (
	...
	maxQueuedHeaders  = 32 * 1024 // [gda/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	// Channels
	headerCh      chan dataPack        // [gda/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [gda/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [gda/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [gda/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [gda/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [gda/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [gda/63] Channel receiving inbound node state data
...
// these are zero.
func (d *Downloader) Progress() gdaereum.SyncProgress {
...
// Synchronising returns whgdaer the downloader is currently retrieving blocks.
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "gda", p.version, "head", hash, "td", td, "mode", d.mode)
...
func (d *Downloader) fetchHeight(p *peerConnection) (*types.Header, error) {
	...
	for {
		...
		case packet := <-d.headerCh:
			...
			// Make sure the peer actually gave somgdaing valid
	...
	for {
		...
		case headers := <-d.headerProcCh:
			...
			if len(headers) == 0 {
				...
				// L: Request new headers up from 11 (R's TD was higher, it must have somgdaing)
				// R: Nothing to give
				if d.mode != LightSync {
			...
			for len(headers) > 0 {
				// Terminate if somgdaing failed in between processing chunks
				select {
				case <-d.cancelCh:
					return errCancelHeaderProcessing
				default:
