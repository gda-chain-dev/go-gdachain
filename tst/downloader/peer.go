...
type peerConnection struct {
	...
	receipgdaarted time.Time // Time instance when the last receipt fetch was started
...
func (p *peerConnection) FetchHeaders(from uint64, count int) error {
	...
	if p.version < 62 {
		panic(fmt.Sprintf("header fetch [gda/62+] requested on gda/%d", p.version))
	}
...
func (p *peerConnection) FetchBodies(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 62 {
		panic(fmt.Sprintf("body fetch [gda/62+] requested on gda/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) FetchReceipts(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("body fetch [gda/63+] requested on gda/%d", p.version))
	}
	// Short circuit if the peer is already fetching
	if !atomic.CompareAndSwapInt32(&p.receiptIdle, 0, 1) {
		return errAlreadyFetching
	}
	p.receipgdaarted = time.Now()
...
func (p *peerConnection) FetchNodeData(hashes []common.Hash) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("node data fetch [gda/63+] requested on gda/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) SetReceiptsIdle(delivered int) {
	p.setIdle(p.receipgdaarted, delivered, &p.receiptThroughput, &p.receiptIdle)
}
...
// Lacks retrieves whgdaer the hash of a blockchain item is on the peers lacking
// list (i.e. whgdaer we know that the peer does not have it).
