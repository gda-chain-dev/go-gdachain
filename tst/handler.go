...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing gdachain peer", "peer", id)

	// Unregister the peer from the downloader and gdachain peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping gdachain protocol")
	...
	log.Info("gdachain protocol stopped")
}
...
// handle is the callback invoked to manage the life cycle of an gda peer. When
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	if err := p.Handshake(pm.networkId, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("gdachain handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("gdachain peer registration failed", "err", err)
		return err
	}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case p.version >= gda63 && msg.Code == GetNodeDataMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= gda63 && msg.Code == NodeDataMsg:
	...
	case p.version >= gda63 && msg.Code == GetReceiptsMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= gda63 && msg.Code == ReceiptsMsg:
...
// NodeInfo represents a short summary of the gdachain sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // gdachain network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
