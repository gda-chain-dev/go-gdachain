...
func newTester(t *testing.T, confOverride func(*gda.Config)) *tester {
	...
	// Create a networkless protocol stack and start an gdachain service within
	...
	gdaConf := &gda.Config{
		...
		gdaerbase: common.HexToAddress(testAddress),
		gdaash: ethash.Config{
			PowMode: ethash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(gdaConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return gda.New(ctx, gdaConf) }); err != nil {
		t.Fatalf("failed to register gdachain protocol: %v", err)
	}
