...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the gdachain
...
type wallet struct {
	...
	deriveChain    gdaereum.ChainStateReader // Blockchain state reader to discover used account with
...
// whgdaer they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived gdachain account
			...
			// If the next account is empty, stop self-derivation, but add it nongdaeless
...
// Contains implements accounts.Wallet, returning whgdaer a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain gdaereum.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
