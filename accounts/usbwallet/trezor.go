...
import (
	...
	"github.com/gdachain/go-gdachain/accounts/usbwallet/internal/trezor"
...
type trezorDriver struct {
	...
	pinwait bool          // Flags whgdaer the device is waiting for PIN entry
...
// Status implements accounts.Wallet, always whgdaer the Trezor is opened, closed
// or whgdaer the gdachain app was not started on it.
...
// gdachain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.gdachainAddress)
	if _, err := w.trezorExchange(&trezor.gdachainGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.gdachainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.gdachainTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
