...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-gda/master/doc/gdaapp.asc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and gdachain address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an gdachain transaction after having the user validate the parameters
...
type ledgerDriver struct {
	...
	browser bool          // Flag whgdaer the Ledger is in browser mode (reply channel mismatch)
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "gdachain app in browser mode", w.failure
	}
	if w.offline() {
		return "gdachain app offline", w.failure
	}
	return fmt.Sprintf("gdachain app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whgdaer the wallet and the gdachain app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// gdachain app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the gdachain app's version, will fail prior to v1.0.2
...
// and returning the gdachain address located on that derivation path.
...
//
// Note, if the version of the gdachain application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested nongdaeless, an error
// will be returned opposed to silently signing in Homestead mode.
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the gdachain app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the gdachain wallet app running
...
// ledgerDerive retrieves the currently active gdachain address from a Ledger
...
//   gdachain address length | 1 byte
//   gdachain address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the gdachain hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an gdachain address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Create the transaction RLP based on whgdaer legacy or EIP155 signing was requeste
	var (
		txrlp []byte
		err   error
	)
	...
	// Extract the gdachain signature and do a sanity validation
	if len(reply) != 65 {
