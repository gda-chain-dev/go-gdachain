...
func WriteTxLookupEntries(db gdadb.Putter, block *types.Block) error {
...
func WriteBloomBits(db gdadb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db gdadb.Database) gdadb.Database {
	return gdadb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db gdadb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteChainConfig(db gdadb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
