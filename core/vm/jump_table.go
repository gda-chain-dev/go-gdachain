...
type operation struct {
	...
	halts   bool // indicates whgdaer the operation should halt further execution
	jumps   bool // indicates whgdaer the program counter should not increment
	writes  bool // determines whgdaer this a state modifying operation
	valid   bool // indication whgdaer the retrieved operation is valid and known
	reverts bool // determines whgdaer the operation reverts state (implicitly halts)
	returns bool // determines whgdaer the operations sets the return data content
}
...
func NewConstantinopleInstructionSet() [256]operation {
	...
	instructionSet[SHL] = operation{
		execute:       opSHL,
		gasCost:       constGasFunc(GasFastesgdaep),
...
func NewFrontierInstructionSet() [256]operation {
	return [256]operation{
		...
		MUL: {
			execute:       opMul,
			gasCost:       constGasFunc(GasFasgdaep),
			validateStack: makeStackFunc(2, 1),
			valid:         true,
		},
		SUB: {
		...
		BLOCKHASH: {
			execute:       opBlockhash,
			gasCost:       constGasFunc(GasExgdaep),
			validateStack: makeStackFunc(1, 1),
			valid:         true,
		},
		COINBASE: {
