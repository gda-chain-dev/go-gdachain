...
// genTxRing returns a block generator that sends gdaer in a ring
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	var db gdadb.Database
	if !disk {
		db, _ = gdadb.NewMemDatabase()
	} else {
		dir, err := ioutil.TempDir("", "gda-core-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		defer os.RemoveAll(dir)
		db, err = gdadb.NewLDBDatabase(dir, 128, 128)
		if err != nil {
...
// into a database.
func makeChainForBench(db gdadb.Database, full bool, count uint64) {
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	for i := 0; i < b.N; i++ {
		dir, err := ioutil.TempDir("", "gda-chain-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		db, err := gdadb.NewLDBDatabase(dir, 128, 1024)
		if err != nil {
