...
// Package core implements the gdachain consensus protocol.
package core
...
type BlockChain struct {
	...
	db     gdadb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default gdachain Validator and
// Processor.
func NewBlockChain(db gdadb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	if cacheConfig == nil {
	...
	if err := bc.loadLasgdaate(); err != nil {
		return nil, err
	}
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= gdadb.IdealBatchSize {
...
// WriteBlockWithougdaate writes only the block and its metadata to the database,
// but does not write any state. This is used to construct competing side forks
// up to the point where they exceed the canonical total difficulty.
func (bc *BlockChain) WriteBlockWithougdaate(block *types.Block, td *big.Int) (err error) {
	bc.wg.Add(1)
	defer bc.wg.Done()
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	// acquiring.
	var (
		stats         = insergdaats{startTime: mclock.Now()}
	...
	for i, block := range chain {
		...
		case err == ErrKnownBlock:
			...
			// this number we did a rollback and we should reimport it nongdaeless.
		...
		case err == consensus.ErrPrunedAncestor:
			...
			if localTd.Cmp(externTd) > 0 {
				if err = bc.WriteBlockWithougdaate(block, externTd); err != nil {
...
// insergdaats tracks and reports on block insertion.
type insergdaats struct {
...
func (st *insergdaats) report(chain []*types.Block, index int, cache common.StorageSize) {
	...
	if index == len(chain)-1 || elapsed >= statsReportLimit {
		...
		*st = insergdaats{startTime: now, lastIndex: index + 1}
	}
}
...
func (bc *BlockChain) reportBlock(block *types.Block, receipts types.Receipts, err error) {
	...
	var receipgdaring string
	for _, receipt := range receipts {
		receipgdaring += fmt.Sprintf("\t%v\n", receipt)
	}
...
Error: %v
##############################
`, bc.chainConfig, block.Number(), block.Hash(), receipgdaring, err))
}
...
//
// The verify parameter can be used to fine tune whgdaer nonce verification
