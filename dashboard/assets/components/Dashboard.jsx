...
// whgdaer the involved data was changed or not by checking the message structure.
//
...
class Dashboard extends Component<Props, State> {
	...
	reconnect = () => {
		...
		server.onopen = () => {
			this.segdaate({content: defaultContent, shouldUpdate: {}});
		};
	...
	changeContent = (newActive: string) => {
		this.segdaate(prevState => (prevState.active !== newActive ? {active: newActive} : {}));
	};

	// switchSideBar opens or closes the sidebar's state.
	switchSideBar = () => {
		this.segdaate(prevState => ({sideBar: !prevState.sideBar}));
	};
