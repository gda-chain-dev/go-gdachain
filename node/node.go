...
func (n *Node) OpenDatabase(name string, cache, handles int) (gdadb.Database, error) {
	if n.config.DataDir == "" {
		return gdadb.NewMemDatabase()
	}
	return gdadb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
