...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.gdastats == "" {
		log.Error("No gdastats server configured")
		return
	}
	...
	if infos.datadir == "" {
		...
		infos.datadir = w.readDefaulgdaring(infos.datadir)
	}
	if w.conf.Genesis.Config.gdaash != nil && !boot {
		fmt.Println()
		if infos.ethashdir == "" {
			...
			infos.ethashdir = w.readDefaulgdaring(infos.ethashdir)
		}
	}
	...
	if infos.gdastats == "" {
		...
		infos.gdastats = w.readString() + ":" + w.conf.gdastats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.gdastats)
		infos.gdastats = w.readDefaulgdaring(infos.gdastats) + ":" + w.conf.gdastats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.gdaash != nil {
			// gdaash based miners only need an gdaerbase to mine against
			fmt.Println()
			if infos.gdaerbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.gdaerbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.gdaerbase)
				infos.gdaerbase = w.readDefaultAddress(common.HexToAddress(infos.gdaerbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy gdachain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
