...
// makeWizard creates and returns a new puppgda wizard.
...
// setting up a new or managing an existing gdachain private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppgda, your gdachain private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new gdachain network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and gdastats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppgda uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering gdachain network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppgda", w.network)
	...
	for {
		...
		default:
			log.Error("That's not somgdaing I can do")
		}
	}
}
