...
// config contains all the configurations needed by puppgda that should be saved
// between sessions.
type config struct {
	path      string   // File containing the configuration values
	bootnodes []string // Bootnodes to always connect to by all nodes
	gdastats  string   // gdastats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save puppgda configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // gdachain services known to be running on servers
...
// readDefaulgdaring reads a single line from stdin, trimming if from spaces. If
// an empty line is entered, the default value is returned.
func (w *wizard) readDefaulgdaring(def string) string {
	fmt.Printf("> ")
	text, err := w.in.ReadString('\n')
	if err != nil {
		log.Crit("Failed to read user input", "err", err)
...
func (w *wizard) readDefaultBigInt(def *big.Int) *big.Int {
	for {
		...
		val, ok := new(big.Int).Segdaring(text, 0)
		if !ok {
...
func (w *wizard) readAddress() *common.Address {
	for {
		...
		bigaddr, _ := new(big.Int).Segdaring(text, 16)
...
// converts it to an gdachain address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by gdastats, which compares IPs textually, not by value.
