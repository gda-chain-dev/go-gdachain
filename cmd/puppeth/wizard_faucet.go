...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many gdaers to release per request? (default = %d)\n", infos.amount)
	...
	if infos.node.datadir == "" {
		...
		infos.node.datadir = w.readDefaulgdaring(infos.node.datadir)
	}
	// Figure out which port to listen on
	fmt.Println()
	...
	fmt.Println()
	if infos.node.gdastats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.gdastats = w.readString() + ":" + w.conf.gdastats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.gdastats)
		infos.node.gdastats = w.readDefaulgdaring(infos.node.gdastats) + ":" + w.conf.gdastats
	}
	...
	infos.noauth = w.readDefaulgdaring(noauth) != "n"
