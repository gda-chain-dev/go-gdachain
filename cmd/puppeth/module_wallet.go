...
FROM puppgda/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'ggda --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'ggda --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --gdastats \'{{.gdastats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*"' >> wallet.sh

RUN \
	sed -i 's/PuppgdaNetworkID/{{.NetworkID}}/g' dist/js/gdaerwallet-master.js && \
	sed -i 's/PuppgdaNetwork/{{.Network}}/g'     dist/js/gdaerwallet-master.js && \
	sed -i 's/PuppgdaDenom/{{.Denom}}/g'         dist/js/gdaerwallet-master.js && \
	sed -i 's/PuppgdaHost/{{.Host}}/g'           dist/js/gdaerwallet-master.js && \
	sed -i 's/PuppgdaRPCPort/{{.RPCPort}}/g'     dist/js/gdaerwallet-master.js
...
// checkWallet does a health-check against web wallet server to verify whgdaer
// it's running, and if yes, whgdaer it's responsive.
