...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"gdastats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "gdastats":
				if infos, err := checkgdastats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have gdastats running, ask whgdaer to make the secret public or not
	if w.conf.gdastats != "" {
		fmt.Println()
		fmt.Println("Include gdastats secret on dashboard (y/n)? (default = yes)")
		infos.trusted = w.readDefaulgdaring("y") == "y"
	}
