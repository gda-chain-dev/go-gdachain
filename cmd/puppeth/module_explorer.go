...
FROM puppgda/explorer:latest

ADD gdastats.json /gdastats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../gda-net-intelligence-api && pm2 start /gdastats.json)' >  explorer.sh && \
	echo '(cd ../gdaerchain-light && npm start &)'                      >> explorer.sh && \
	echo '/parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// explorergdastats is the configuration file for the gdastats javascript client.
var explorergdastats = `[
  {
    "name"              : "node-app",
    "script"            : "app.js",
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.gdaereum
    environment:
      - NODE_PORT={{.NodePort}}/tcp
      - STATS={{.gdastats}}{{if .VHost}}
      - VIRTUAL_HOST={{.VHost}}
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	gdastats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorergdastats)).Execute(gdastats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.gdastats[:strings.Index(config.gdastats, ":")],
		"Secret": config.gdastats[strings.Index(config.gdastats, ":")+1 : strings.Index(config.gdastats, "@")],
		"Host":   config.gdastats[strings.Index(config.gdastats, "@")+1:],
	})
	files[filepath.Join(workdir, "gdastats.json")] = gdastats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.gdaereum"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		gdastats: infos.envvars["STATS"],
	}
	return stats, nil
}
