...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for gdastats availability")
	if infos, err := checkgdastats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["gdastats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["gdastats"] = infos.Report()
		gdastats = infos.config
	}
