...
func TestMessageSignVerify(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "gdakey-test")
	if err != nil {
	...
	// Create the key.
	generate := rungdakey(t, "generate", keyfile)
	generate.Expect(`
...
`)
	...
	// Sign a message.
	sign := rungdakey(t, "signmessage", keyfile, message)
	sign.Expect(`
!! Unsupported terminal, password will be echoed.
Passphrase: {{.InputLine "foobar"}}
`)
	...
	// Verify the message.
	verify := rungdakey(t, "verifymessage", address, signature, message)
