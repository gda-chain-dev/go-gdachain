...
// faucet is a gdaer faucet backed by a light client.
package main
...
var (
	...
	gdaPortFlag = flag.Int("gdaport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the gdachain protocol")
	statsFlag   = flag.String("gdastats", "", "gdastats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of gdaers to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for gdachain and the faucet")
)

var (
	gdaer = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s gdaers", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *gdaPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // gdachain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an gdachain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // gdachain protocol stack
	client *gdaclient.Client   // Client connection to the gdachain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	stack, err := node.New(&node.Config{
		...
		P2P: p2p.Config{
			...
			MaxPeers:         25,
			BoogdarapNodesV5: enodes,
		},
	})
	if err != nil {
		return nil, err
	}
	// Assemble the gdachain light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := gda.DefaultConfig
	...
	// Assemble the gdastats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
	...
	client := gdaclient.NewClient(api)
...
// close terminates the gdachain connection and tears down the faucet.
...
// apiHandler handles requests for gdaer grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, gdaer),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the gdachain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Somgdaing funky happened, please open an issue at https://github.com/gdachain/go-gdachain/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), gdaer)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance = new(big.Int).Div(balance, gdaer)
...
// the username, avatar URL and gdachain address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for gdachain addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No gdachain address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the gdachain address and profile URL.
...
// returning the username, avatar URL and gdachain address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain gdachain address,
