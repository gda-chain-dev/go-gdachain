...
import (
	...
	"github.com/gdachain/go-gdachain/console"
	...
	"github.com/gdachain/go-gdachain/swarm"
	bzzapi "github.com/gdachain/go-gdachain/swarm/api"
	swarmmetrics "github.com/gdachain/go-gdachain/swarm/metrics"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:   "swap-api",
		Usage:  "URL of the gdachain API provider to use to settle SWAP payments",
	...
	DeprecatedgdaAPIFlag = cli.StringFlag{
		Name:  "ethapi",
...
// This init function sets defaults so cmd/swarm can run alongside ggda.
func init() {
...
var app = utils.NewApp(gitCommit, "gdachain Swarm")
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.gdaereum/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.gdaereum/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:    dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.gdaereum/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.gdaereum/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:    dbClean,
...
func bzzd(ctx *cli.Context) error {
	...
	//ggda only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for ggda and swarm
	if _, err := os.Stat(bzzconfig.Path); err == nil {
		cfg.DataDir = bzzconfig.Path
	}
	//setup the gdaereum node
	...
	//register BZZ as node.Service in the gdaereum node
...
func registerBzzService(bzzconfig *bzzapi.Config, ctx *cli.Context, stack *node.Node) {
	...
	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		var swapClient *gdaclient.Client
		var err error
		if bzzconfig.SwapApi != "" {
			log.Info("connecting to SWAP API", "url", bzzconfig.SwapApi)
			swapClient, err = gdaclient.Dial(bzzconfig.SwapApi)
			if err != nil {
	...
	//register within the gdaereum node
...
func getAccount(bzzaccount string, ctx *cli.Context, stack *node.Node) *ecdsa.PrivateKey {
	...
	return decrypgdaoreAccount(ks, bzzaccount, utils.MakePasswordList(ctx))
}

func decrypgdaoreAccount(ks *keystore.KeyStore, account string, passwords []string) *ecdsa.PrivateKey {
