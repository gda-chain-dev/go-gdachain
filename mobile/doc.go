...
// Package ggda contains the simplified mobile APIs to go-gdaereum.
//
// The scope of this package is *not* to allow writing a custom gdachain client
// with pieces plucked from go-gdaereum, rather to allow writing native dapps on
...
// from the go-gdaereum repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original go-gdaereum version.
