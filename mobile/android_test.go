...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Ggda.LightScryptN, Ggda.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.gdaereum", new NodeConfig());
			node.start();
			...
			gdachainClient ec = node.getgdachainClient();
	...
	// signatures too. Regression test for go-gdaereum issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altoggdaer
	...
	workspace, err := ioutil.TempDir("", "ggda-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/gdaereum/ggdatest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Ggda and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.gdaereum", "github.com/gdachain/go-gdachain/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "ggda.aar"), "ggda.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "gdaereum", "ggdatest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
