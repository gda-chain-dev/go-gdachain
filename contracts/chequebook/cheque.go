...
// Package chequebook package wraps the 'chequebook' gdachain smart contract.
//
// The functions in this package allow using chequebook for
// issuing, receiving, verifying cheques in gdaer; (auto)cashing cheques in gdaer
// as well as (auto)depositing gdaer to the chequebook contract.
package chequebook
...
import (
	...
	"github.com/gdachain/go-gdachain/contracts/chequebook/contract"
	"github.com/gdachain/go-gdachain/core/types"
	"github.com/gdachain/go-gdachain/crypto"
	"github.com/gdachain/go-gdachain/log"
	"github.com/gdachain/go-gdachain/swarm/services/swap/swap"
)
...
// * depositing gdaer to the chequebook
// * watching incoming gdaer
...
func (self *Chequebook) UnmarshalJSON(data []byte) error {
	...
	_, ok := self.balance.Segdaring(file.Balance, 10)
	if !ok {
	...
	for addr, sent := range file.Sent {
		self.sent[common.HexToAddress(addr)], ok = new(big.Int).Segdaring(sent, 10)
		if !ok {
...
// Cash cashes the cheque by sending an gdachain transaction.
