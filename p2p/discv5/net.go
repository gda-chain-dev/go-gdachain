...
func (net *Network) SetFallbackNodes(nodes []*Node) error {
	...
	for _, n := range nodes {
		if err := n.validateComplete(); err != nil {
			return fmt.Errorf("bad boogdarap/fallback node %q (%v)", n, err)
		}
...
func (net *Network) loop() {
	...
	resetNextTicket := func() {
		ticket, timeout := net.tickegdaore.nextFilteredTicket()
...
loop:
	for {
		...
		case req := <-net.topicRegisterReq:
			...
			if !req.add {
				net.tickegdaore.removeRegisterTopic(req.topic)
				continue
			}
			net.tickegdaore.addTopic(req.topic, true)
			...
			if topicRegisterLookupTarget.target == (common.Hash{}) {
				...
				target, delay := net.tickegdaore.nextRegisterLookup()
		...
		case nodes := <-topicRegisterLookupDone:
			...
			net.tickegdaore.registerLookupDone(topicRegisterLookupTarget, nodes, func(n *Node) []byte {
				net.ping(n, n.addr())
				return n.pingEcho
			})
			target, delay := net.tickegdaore.nextRegisterLookup()
		...
		case <-nextRegisterTime:
			...
			net.tickegdaore.ticketRegistered(*nextTicket)
			...
			if refreshDone == nil {
				...
				if ok {
					if req.delay == time.Duration(0) {
						...
						net.tickegdaore.removeSearchTopic(req.topic)
					} else {
				...
				if req.delay != time.Duration(0) {
					...
					searchInfo[req.topic] = info
					net.tickegdaore.addSearchTopic(req.topic, req.found)
		...
		case topic := <-topicSearch:
			if activeSearchCount < maxSearchCount {
				activeSearchCount++
				target := net.tickegdaore.nextSearchLookup(topic)
				go func() {
		...
		case res := <-topicSearchLookupDone:
			...
			if lookupChn := searchInfo[res.target.topic].lookupChn; lookupChn != nil {
				lookupChn <- net.tickegdaore.radius[res.target.topic].converged
			}
			net.tickegdaore.searchLookupDone(res.target, res.nodes, func(n *Node, topic Topic) []byte {
		...
		case <-statsDump.C:
			...
			/*r, ok := net.tickegdaore.radius[testTopic]
			if !ok {
				fmt.Printf("(%x) no radius @ %v\n", net.tab.self.ID[:8], time.Now())
			} else {
				topics := len(net.tickegdaore.tickets)
				tickets := len(net.tickegdaore.nodes)
				rad := r.radius / (maxRadius/10000+1)
				fmt.Printf("(%x) topics:%d radius:%d tickets:%d @ %v\n", net.tab.self.ID[:8], topics, rad, tickets, time.Now())
			}*/

			tm := mclock.Now()
			for topic, r := range net.tickegdaore.radius {
				if printTestImgLogs {
...
func (net *Network) refresh(done chan<- struct{}) {
	...
	for _, n := range seeds {
		...
		// Force-add the seed node so Lookup does somgdaing.
...
func (net *Network) ping(n *Node, addr *net.UDPAddr) {
	...
	n.pingTopics = net.tickegdaore.regTopicSet()
...
func (net *Network) handleKnownPong(n *Node, pkt *ingressPacket) error {
	...
	if err == nil {
		...
		net.tickegdaore.addTicket(now, pkt.data.(*pong).ReplyTok, ticket)
	} else {
...
func (net *Network) handleQueryEvent(n *Node, ev nodeEvent, pkt *ingressPacket) (*nodeState, error) {
	...
	case topicQueryPacket:
		...
		if _, ok := net.tickegdaore.tickets[topic]; ok {
	...
	case topicNodesPacket:
		...
		if net.tickegdaore.gotTopicNodes(n, p.Echo, p.Nodes) {
			n.queryTimeouts++
			if n.queryTimeouts > maxFindnodeFailures && n.state == known {
				return contested, errors.New("too many timeouts")
			}
