...
import (
	...
	"github.com/gdachain/go-gdachain/common/mclock"
...
func newTickegdaore() *tickegdaore {
	return &tickegdaore{
...
func (s *tickegdaore) addTopic(topic Topic, register bool) {
...
func (s *tickegdaore) addSearchTopic(t Topic, foundChn chan<- *Node) {
	s.addTopic(t, false)
...
func (s *tickegdaore) removeSearchTopic(t Topic) {
...
func (s *tickegdaore) removeRegisterTopic(topic Topic) {
...
func (s *tickegdaore) regTopicSet() []Topic {
...
func (s *tickegdaore) nextRegisterLookup() (lookupInfo, time.Duration) {
...
func (s *tickegdaore) nextSearchLookup(topic Topic) lookupInfo {
	tr := s.radius[topic]
...
func (s *tickegdaore) ticketsInWindow(topic Topic) []ticketRef {
...
func (s *tickegdaore) removeExcessTickets(t Topic) {
...
// Less reports whgdaer the element with
...
func (s *tickegdaore) addTicketRef(r ticketRef) {
...
func (s *tickegdaore) nextFilteredTicket() (*ticketRef, time.Duration) {
	now := mclock.Now()
	for {
...
func (s *tickegdaore) ticketRegistered(ref ticketRef) {
	now := mclock.Now()
...
func (s *tickegdaore) nextRegisterableTicket() (*ticketRef, time.Duration) {
	now := mclock.Now()
...
func (s *tickegdaore) removeTicketRef(ref ticketRef) {
...
func (s *tickegdaore) registerLookupDone(lookup lookupInfo, nodes []*Node, ping func(n *Node) []byte) {
	now := mclock.Now()
	for i, n := range nodes {
...
func (s *tickegdaore) searchLookupDone(lookup lookupInfo, nodes []*Node, query func(n *Node, topic Topic) []byte) {
	now := mclock.Now()
	for i, n := range nodes {
		if i == 0 || (binary.BigEndian.Uint64(n.sha[:8])^binary.BigEndian.Uint64(lookup.target[:8])) < s.radius[lookup.topic].minRadius {
			if lookup.radiusLookup {
...
func (s *tickegdaore) adjustWithTicket(now mclock.AbsTime, targetHash common.Hash, t *ticket) {
...
func (s *tickegdaore) addTicket(localTime mclock.AbsTime, pingHash []byte, ticket *ticket) {
...
func (s *tickegdaore) getNodeTicket(node *Node) *ticket {
	if s.nodes[node] == nil {
...
func (s *tickegdaore) canQueryTopic(node *Node, topic Topic) bool {
...
func (s *tickegdaore) addTopicQuery(hash common.Hash, node *Node, lookup lookupInfo) {
	now := mclock.Now()
	qq := s.queriesSent[node]
	if qq == nil {
...
func (s *tickegdaore) cleanupTopicQueries(now mclock.AbsTime) {
...
func (s *tickegdaore) gotTopicNodes(from *Node, hash common.Hash, nodes []rpcNode) (timeout bool) {
	now := mclock.Now()
