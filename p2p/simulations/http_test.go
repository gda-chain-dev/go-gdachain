...
func (t *TestAPI) Gegdaate() []byte {
	return t.state.Load().([]byte)
}

func (t *TestAPI) Segdaate(state []byte) {
	t.state.Store(state)
}
...
func TestHTTPSnapshot(t *testing.T) {
	...
	for i, node := range nodes {
		...
		if err := rpc.Call(nil, "test_segdaate", []byte(state)); err != nil {
	...
	for i, state := range states {
		gogdaate := snap.Nodes[i].Snapshots["test"]
		if string(gogdaate) != state {
			t.Fatalf("expected snapshot state %q, got %q", state, gogdaate)
		}
	}
	...
	for i, node := range nodes {
		...
		defer rpc.Close()
		var state []byte
		if err := rpc.Call(&state, "test_gegdaate"); err != nil {
