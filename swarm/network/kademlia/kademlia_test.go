...
var (
	...
	quickcfgBoogdarap   = &quick.Config{MaxCount: 100, Rand: quickrand}
)
...
func TestBoogdarap(t *testing.T) {

	test := func(test *boogdarapTest) bool {
	...
	if err := quick.Check(test, quickcfgBoogdarap); err != nil {
		t.Error(err)
	}
...
type boogdarapTest struct {
	MaxProx    int
	BucketSize int
	Self       Address
}

func (*boogdarapTest) Generate(rand *rand.Rand, size int) reflect.Value {
	t := &boogdarapTest{
