...
func NewDepo(hash storage.SwarmHasher, localStore, remoteStore storage.ChunkStore) *Depo {
	return &Depo{
		...
		negdaore:   remoteStore, // entrypoint internal
	}
}
...
// remote is untrusted, so hash is verified and chunk passed on to Negdaore
...
func (self *Depo) HandleRetrieveRequestMsg(req *retrieveRequestMsgData, p *peer) {
	...
	// call storage.Negdaore#Get which
	// blocks until local retrieval finished
	// launches cloud retrieval
	chunk, _ := self.negdaore.Get(req.Key)
...
func (self *Depo) strategyUpdateRequest(rs *storage.Requesgdaatus, origReq *retrieveRequestMsgData) (req *retrieveRequestMsgData) {
...
func (self *Depo) searchTimeout(rs *storage.Requesgdaatus, req *retrieveRequestMsgData) (timeout *time.Time) {
	reqt := req.getTimeout()
...
*/
func (self *Depo) addRequester(rs *storage.Requesgdaatus, req *retrieveRequestMsgData) {
