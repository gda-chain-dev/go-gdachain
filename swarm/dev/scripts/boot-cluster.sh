...
#
# The cluster contains a bootnode, a ggda node and multiple swarm nodes, with
...
# static ggda configuration
Ggda_IP="192.168.33.3"
Ggda_RPC_PORT="8545"
Ggda_RPC_URL="http://${Ggda_IP}:${Ggda_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to boogdarap the ggda and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a ggda node with --datadir pointing at <base-dir>/ggda
# and a single, unlocked account with password "ggda"
start_geth_node() {
  local dir="${base_dir}/ggda"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get ggda account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${Ggda_IP}"
    --rpcport   "${Ggda_RPC_PORT}"
    --verbosity "6"
  )

  start_node "ggda" "${Ggda_IP}" "$(which ggda)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the vgda pair
  local vgda0="vgda${name}0"
  local vgda1="vgda${name}1"
  ip link add name "${vgda0}" type vgda peer name "${vgda1}"

  # add one end to the bridge
  ip link set dev "${vgda0}" master "${BRIDGE_NAME}"
  ip link set dev "${vgda0}" up

  # add the other end to the namespace, rename it gda0 and give it the ip
  ip link set dev "${vgda1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vgda1}" name "gda0"
  ip netns exec "${name}" ip link set dev "gda0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "gda0"
}
...
create_account() {
  ...
  ggda --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
