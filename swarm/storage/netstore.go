...
/*
Negdaore is a cloud storage access abstaction layer for swarm
...
Negdaore falls back to a backend (CloudStorage interface)
implemented by bzz/network/forwarder. forwarder or IPFS or IPΞS
*/
type Negdaore struct {
...
func NewDefaulgdaoreParams() (self *StoreParams) {
	return &StoreParams{
...
// negdaore contructor, takes path argument that is used to initialise dbStore,
...
func NewNegdaore(hash SwarmHasher, lstore *LocalStore, cloud CloudStore, params *StoreParams) *Negdaore {
	return &Negdaore{
		hashfunc:   hash,
		localStore: lstore,
		cloud:      cloud,
...
func (self *Negdaore) Put(entry *Chunk) {
	self.localStore.Put(entry)

	// handle deliveries
	if entry.Req != nil {
		log.Trace(fmt.Sprintf("Negdaore.Put: localStore.Put %v hit existing request...delivering", entry.Key.Log()))
		...
		log.Trace(fmt.Sprintf("Negdaore.Put: localStore.Put %v stored locally", entry.Key.Log()))
...
func (self *Negdaore) Get(key Key) (*Chunk, error) {
	var err error
	chunk, err := self.localStore.Get(key)
	if err == nil {
		if chunk.Req == nil {
			log.Trace(fmt.Sprintf("Negdaore.Get: %v found locally", key))
		} else {
			log.Trace(fmt.Sprintf("Negdaore.Get: %v hit on an existing request", key))
			// no need to launch again
		}
		return chunk, err
	}
	// no data and no request status
	log.Trace(fmt.Sprintf("Negdaore.Get: %v not found locally. open new request", key))
	chunk = NewChunk(key, newRequesgdaatus(key))
