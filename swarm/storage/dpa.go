...
// Chunk retrieval blocks on negdaore requests with a timeout so reader will
...
// retrieve channel to its ChunkStore  (Negdaore or LocalStore)
...
// received on the store channel to its ChunkStore (Negdaore or LocalStore)
...
// local storage eg. LocalStore and network storage eg., Negdaore
...
func NewDpaChunkStore(localStore, negdaore ChunkStore) *dpaChunkStore {
	return &dpaChunkStore{0, localStore, negdaore}
}

// Get is the entrypoint for local retrieve requests
// waits for response or times out
func (self *dpaChunkStore) Get(key Key) (chunk *Chunk, err error) {
	chunk, err = self.negdaore.Get(key)
...
func (self *dpaChunkStore) Put(entry *Chunk) {
	...
	if err != nil {
		log.Trace(fmt.Sprintf("DPA.Put: %v new chunk. call negdaore.Put", entry.Key.Log()))
		chunk = entry
