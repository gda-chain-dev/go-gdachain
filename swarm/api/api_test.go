...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.gda"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.gda"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.gda"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	gdaAddr := "swarm.gda"
	gdaHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	gdaResolve := newTestResolver(gdaHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(gdaResolve, "")),
			addr:   gdaAddr,
			result: gdaHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(gdaResolve, ""),
				MultiResolverOptionWithResolver(gdaResolve, ""),
			),
			addr:   gdaAddr,
			result: gdaHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(gdaResolve, "gda"),
			),
			addr:   gdaAddr,
			result: gdaHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "gda"),
				MultiResolverOptionWithResolver(doesntResolve, "gda"),
				MultiResolverOptionWithResolver(gdaResolve, "gda"),
			),
			addr:   gdaAddr,
		...
		},
		{
			...
			addr:   gdaAddr,
			result: gdaHash,
			err:    errors.New(`DNS name not found: "swarm.gda"`),
		},
		{
